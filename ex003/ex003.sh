...
# Unix progs:	gawk, cat, echo, head, paste, rm, tail
#
...
# The main illustration (ex003.ps) are accompanied with 5 support
...
gmt psxy $R -UL/-1.75i/-1.25i/"Example 3a in Cookbook" -BWeSn \
	...
	-JX8i/5i -X2i -Y1.5i -K -Wthick sat.pg > ex003a.ps
gmt psxy -R -JX -O -Sp0.03i ship.pg >> ex003a.ps
#
...
# typically spaced using gawk to get the delta-p between points and view it with
# "gmt pshistogram".
#
gawk '{ if (NR > 1) print $1 - last1; last1=$1; }' ship.pg | gmt pshistogram  -W0.1 -Gblack \
	-JX3i -K -X2i -Y1.5i -B0 -B+t"Ship" -UL/-1.75i/-1.25i/"Example 3b in Cookbook" \
	> ex003b.ps
gawk '{ if (NR > 1) print $1 - last1; last1=$1; }' sat.pg  | gmt pshistogram  -W0.1 -Gblack \
	-JX3i -O -X5i -B0 -B+t"Sat" >> ex003b.ps
#
...
gmt psxy $R -JX8i/5i -X2i -Y1.5i -K -Wthick samp_sat.pg \
	-Bxa500f100+l"Distance along great circle" -Bya100f25+l"Gravity anomaly (mGal)" \
	-BWeSn -UL/-1.75i/-1.25i/"Example 3c in Cookbook" > ex003c.ps
gmt psxy -R -JX -O -Sp0.03i samp_ship.pg >> ex003c.ps
#
...
# We could throw away a section of data with gawk or sed or head and tail, but we
...
# de-trended data):
gmt trend1d -Fxrw -Np1+r samp_ship.pg | gawk '{ if ($3 > 0.6) print $1, $2 }' \
	| gmt sample1d -Nsamp.x > samp2_ship.pg
gmt trend1d -Fxrw -Np1+r samp_sat.pg  | gawk '{ if ($3 > 0.6) print $1, $2 }' \
...
gmt psxy $R -JX8i/5i -X2i -Y1.5i -K -Wthick \
	...
	-BWeSn -UL/-1.75i/-1.25i/"Example 3e in Cookbook" samp2_sat.pg > ex003e.ps
gmt psxy -R -JX -O -Sp0.03i samp2_ship.pg >> ex003e.ps
#
# Now we do the cross-spectral analysis again.  Comparing this plot (ex003e.ps) with
# the previous one (ex003d.ps) we see that throwing out the large feature has reduced
...
gmt psxy spectrum.coh -Bxa1f3p+l"Wavelength (km)" -Bya0.25f0.05+l"Coherency@+2@+" -BWeSn \
	...
	-Sc0.07i -Gblack -Ey/0.5p -Y1.5i > ex003f.ps
...
4	3.25
END
gmt psxy -R -Jx -O -K -Wthicker box.d >> ex003f.ps
gmt psxy -Bxa1f3p -Bya1f3p+l"Power (mGal@+2@+km)" -BWeSn+t"Ship and Satellite Gravity" \
	...
	>> ex003f.ps
gmt psxy spectrum.ypower -R -JX -O -K -Gblack -Sc0.07i -Ey/0.5p >> ex003f.ps
echo "3.9 3.6 Input Power" | gmt pstext -R0/4/0/3.75 -Jx -F+f18p,Helvetica-Bold+jTR -O \
	-K >> ex003f.ps
gmt psxy -R -Jx -O -K -Wthicker box.d >> ex003f.ps
gmt psxy -R -Jx -O -K -Glightgray -L -Wthicker >> ex003f.ps << END
0.25	0.25
1.4	0.25
1.4	0.9
0.25	0.9
END
echo "0.4 0.7" | gmt psxy -R -Jx -O -K -ST0.07i -Gblack >> ex003f.ps
echo "0.5 0.7 Ship" | gmt pstext -R -Jx -F+f14p,Helvetica-Bold+jLM -O -K >> ex003f.ps
echo "0.4 0.4" | gmt psxy -R -Jx -O -K -Sc0.07i -Gblack >> ex003f.ps
echo "0.5 0.4 Satellite" | gmt pstext -R -Jx -F+f14p,Helvetica-Bold+jLM -O >> ex003f.ps
#
